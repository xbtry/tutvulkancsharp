...
unsafe class VulkanTutorialApp
{
    ...
    private void CleanUp()
    {
        ...
        File.Delete(Path.Combine(projectDir,"vshader.spv"));
        File.Delete(Path.Combine(projectDir,"fshader.spv"));
        ...
        )
    {
        ...
        {
            if (_vk!.CreateBuffer(_device, in bufferCreateInfo, null, bufferPtr) != Result.Success)
            {
        ...
        {
            if (_vk!.AllocateMemory(_device, in memoryAllocateInfo, null, bufferMemoryPtr) != Result.Success)
            {
    ...
    private void CopyBuffer(Buffer srcBuffer, Buffer dstBuffer, ulong size)
    {
        ...
        _vk!.FreeCommandBuffers(_device, _graphicsCommandPool, 1, in commandBuffer);
    }
    ...
    {
        ulong bufferSize = (ulong)(Unsafe.SizeOf<Index>() * indices.Length);
        Buffer stagingBuffer = default;
        DeviceMemory stagingBufferMemory = default;
        CreateBuffer(bufferSize, BufferUsageFlags.TransferSrcBit, MemoryPropertyFlags.HostVisibleBit | MemoryPropertyFlags.HostCoherentBit, ref stagingBuffer, ref stagingBufferMemory);
    ...
    {
        _vk!.GetPhysicalDeviceMemoryProperties(_physicalDevice, out PhysicalDeviceMemoryProperties physicalDeviceMemoryProperties);
    ...
    private void CreateCommandBuffers()
    {
        ...
        for (int i = 0; i < _graphicsCommandBuffers.Length; i++)
        {
            ...
            {
                Color = new ClearColorValue() { Float32_0 = 0.3f, Float32_1 = 0.2f, Float32_2 = 0.4f, Float32_3 = 1.0f }
            };
            ...
            {
                _vk!.CmdBindVertexBuffers(_graphicsCommandBuffers[i], 0, 1, vertexBuffersPtr, offsetsPtr);
            }
            
            _vk!.CmdBindIndexBuffer(_graphicsCommandBuffers[i],_indexBuffer,0,IndexType.Uint16);
            
            _vk!.CmdDrawIndexed(_graphicsCommandBuffers[i], (uint)indices.Length, 1, 0 , 0, 0);
